...
The primary reading copy is the Internet Archive's scan of the typescript in
*Miscellaneous Writings*:

- [Typescript scan, beginning at *Creative Thinking*](https://archive.org/details/ShannonMiscellaneousWritings/page/n829/mode/2up)
- [Internet Archive item record](https://archive.org/details/ShannonMiscellaneousWritings)
- [Internet Archive full OCR text](https://archive.org/stream/ShannonMiscellaneousWritings/ShannonMiscellaneousWritings_djvu.txt)

The dates describe distinct events.
In 1990, N. J. A. Sloane and Aaron D. Wyner received Shannon's permission to assemble his collected papers.
...
For easier reading, [James Clear](https://jamesclear.com/great-speeches/creative-thinking-by-claude-shannon)
and [Farnam Street](https://fs.blog/great-talks/creative-thinking-claude-shannon/)
provide reflowed web transcripts. They are secondary transcriptions, not the
primary typescript or separate publications of the talk.

## Source-derived

Shannon first discusses training and experience, intelligence, and motivation.
On motivation he invokes Fats Waller's remark about swing music, quoted by
Shannon as “either you got it or you ain't.” He also describes constructive
dissatisfaction and the mental ruts that can keep an experienced researcher
circling one viewpoint. He then presents several conscious aids for finding a
fresh route through a problem.

### Simplification

**Source-derived core:** eliminate everything except the essentials, even when
the result barely resembles the original problem. Shannon says that, often,
one can solve the simple problem and add refinements until returning to a
...
**Source-derived core:** state the problem in many forms, change the words and
viewpoint, and consider several angles to escape a mental rut and reveal the
basic issues.

**Authored extension:** the collection's invariant ledger, minimum of three
materially different representations, and explicit labels for intentional
relaxations are modern controls against silently changing the problem.

### Generalization / Generalize

**Source-derived core:** broaden an already-found result or principle. Shannon
describes taking a result from two dimensions to `N` dimensions, moving to a
larger algebraic setting, and asking where else an already successful idea can
...
**Source-derived core:** replace one large jump with a path through subsidiary
theorems, propositions, partial results, or a deliberately clumsy first
design, then use those intermediate results to reach and simplify the final
solution.

**Authored extension:** the collection compares candidate seams, rejects weak
ones, converts pieces into answerable questions, orders them by information
yield and dependency, and checks recomposition against cross-cutting
constraints.

### Inversion / Invert

**Source-derived core:** exchange premises and result, reason from an assumed
`S` toward `P`, and, when possible, invert the route in smaller stages into a
forward solution.

**Authored extension:** the collection represents backward reasoning as a branching
causal graph. It labels necessary versus sufficient conditions, non-invertible
edges, unknown sufficiency, and alternative predecessors, then requires a
forward replay from the actual current state.
...
Across the lenses, modern authored extensions make transformations auditable:
